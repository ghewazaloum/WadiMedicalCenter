...
const EditTherapist = () => {
 ...
 // console.log(location.state.DoctorAndTherapist,location.state.doctorName)
 ...
 const[startHoursIn,setstartHoursIn]=useState('');
 const[endHoursIn,setendHoursIn]=useState('')
...
React.useEffect(()=> {
  if(headers.Authorization!=='Token undefined'){
    ...
    if(DoctorName!==''){
      if(headers.Authorization!=='Token undefined'){
       axios.post(`${BASE_URL}appmanager/therapist_information`,formdata,{headers}).then((res)=>{
        console.log('therapist info',res);
        if(res.data.result!=="invalid"){
          infoToEdit=res.data;
      if(infoToEdit!=null){
        setDoctorSection(infoToEdit.therapistSection);
        setDoctorDescription(infoToEdit.therapistDesc);
        ...
        setDoctorImage(`${BASE_URL}${infoToEdit.therapistImage}`);
  ...
  const getDoctorImage=(e)=>{
    if(e.target.files && e.target.files[0]){
      ...
      console.log(URL.createObjectURL(e.target.files[0]),"hhhh")
...
const handleDoctorSection=(val)=>(e)=>{
  ...
  const EditDoctor=(e)=>{
    ...
    e.preventDefault();
    let formdata=new FormData();
    formdata.append('therapistName',DoctorName);
    formdata.append('therapistImage',DoctorImagetosend);
    formdata.append('spezialiation',DoctorSpecialization);
    ...
    //console.log(doctorName,doctorImage,Specialization,description,sectionName,saturday,sunday,monday,tuesday,wednesday,
     ...
     if(DoctorName!==''&&DoctorSection!==null&&startHoursIn!=='00:00'&&endHoursIn!=='00:00'&&(saturday!=='0'||sunday!=='0'||monday!=='0'||tuesday!=='0'||wednesday!=='0'||thursday!=='0'||friday!=='0')){
      if(headers.Authorization!=='Token undefined'){
      axios.post(`${BASE_URL}appmanager/edit_therapist`,formdata,{headers}).then((res)=>{
...
const DeleteTherapist=(name)=>(e)=>{
  ...
  return (
    ...
    <div className="EditTherapist">
      ...
      <RightSideBar>
        <LeftSideBarHeading>Edit Therapist</LeftSideBarHeading>
        ...
        <div className="TechnicanList">
          ...
          <li key={i}>
            {val!==DoctorSection?
           <a className='TechnicanNotSelected' onClick={handleDoctorSection({val})}>{val}</a>:<a className='TechnicanSelected' onClick={handleDoctorSection({val})}>{val}</a>}
          </li> 
         )} 
       </ul> 
        </>:null}
