...
const AddDoctor = () => {
 ...
 {/*  let DoctorAndTherapist=null;
  ...
  const[DoctorAndTherapist,setDoctorAndTherapist]=useState(null);
...
React.useEffect(()=> {
  ...
  axios.get(`${BASE_URL}appmanager/doctors_therapists`,{headers}).then((res) => {
    if(res.data.result==="ok"){
      setDoctorAndTherapist(res.data);
}
  })};
}, []);
//saturday
...
//save the value of Doctor name in a variable
...
const add_doctor=(e)=>{
  ...
  formdata.append('endHoursIn',endHoursIn);
   if(DoctorName!==''&&DoctorSection!==null&&startHoursIn!=='00:00'&&endHoursIn!=='00:00'&&(saturday!=='0'||sunday!=='0'||monday!=='0'||tuesday!=='0'||wednesday!=='0'||thursday!=='0'||friday!=='0')){
    if(headers.Authorization!=='Token undefined'){
    axios.post(`${BASE_URL}appmanager/add_doctor`,formdata,{headers}).then((res)=>{
...
const navigateToAddTherapist=(e)=>{
  e.preventDefault();
  if(DoctorAndTherapist!==null){
  console.log('we will pass it to next page',DoctorAndTherapist);
  navigate('/AddTherapist', { replace: true, state: {DoctorAndTherapist:DoctorAndTherapist}})
  }}  
...
//Delete function and disappear from dom
const DeleteTherapist=(name)=>(e)=>{
  e.preventDefault();
  const headers = { 'Authorization': `Token ${cookie.get('Token')}` };
  const doctorName=name;
  axios.post(`${BASE_URL}appmanager/delete_therapist`,{
    doctorName
     },{headers}).then(res=>{
   console.log('deleted',res);
   if(res.data.result==="ok"){
...
//function to send data to Edit Doctor
const navigateToEditDoctor=(doctorName)=>(e)=>{
  e.preventDefault();
  if(DoctorAndTherapist!==null){
   console.log('we will pass it to next page',DoctorAndTherapist);
 navigate('/EditDoctor', { replace: true, state: {doctorName:doctorName,DoctorAndTherapist:DoctorAndTherapist}})
}}
//function to send data to Edit device Doctor
const navigateToEditTherapist=(doctorName)=>(e)=>{
  e.preventDefault();
  if(DoctorAndTherapist!==null){
   console.log('we will pass it to next page',DoctorAndTherapist);
 navigate('/EditTherapist', { replace: true, state: {doctorName:doctorName,DoctorAndTherapist:DoctorAndTherapist}})
}}
  return (
    <div className='AddDoctor'>
       ...
       <tr key={i}>
            ...
            <td></td>
             <td >
            <button className='appmanagerEditButton'onClick={navigateToEditDoctor(val.Name)}><a href=''>Edit</a></button>
            </td>
            <td >
        ...
        <div style={{display:"flex"}}>
        <TableHeading>Therapists</TableHeading>
          <button className='addTherapist'>
            <a href='/AddTherapist'onClick={navigateToAddTherapist}>
             Add
            </a>
            </button>
        ...
        <Table zero="Name"  first="Section" second="specialization" >
        {DoctorAndTherapist!==null?
        DoctorAndTherapist.Therapists.map((val,i)=>
       <tr key={i}>
            <td>{val.Name}</td>
            <td>{val.Section}</td>
            <td>{val.Specialization}</td>
            ...
            <button className='appmanagerEditButton'>
              <a href=''onClick={navigateToEditTherapist(val.Name)}>
                Edit
                </a>
                </button>
            </td>
            <td >
            <button className='DeleteButton'>
            <a href='#'onClick={DeleteTherapist(val.Name)}>
            Delete
            </a>
            </button>
            </td>
      ...
      <RightSideBar>
        <LeftSideBarHeading>New Doctor</LeftSideBarHeading>
          <div className='rightSideBarAppManager'>
            {/*...................name input..................... */}
            <SideBarDivText>Name:</SideBarDivText>
            <input id="input" placeholder='Enter Name...' type='text' name='name' onChange={handleDoctorName} required/>
        ...
        <div className="TechnicanList">
          ...
          <li key={i} >
           <a className='TechnicanNotSelected'onClick={handleDoctorSection({val})}>{val}</a>
            ...
            <input id="input" placeholder='Enter Specialization...' type='text' name='Specialization' onChange={handleDoctorSpecialization} required/>
            {/*...................description input..................... */}
            <SideBarDivText>Description:</SideBarDivText>
            <textarea className='textarea' rows="5" placeholder='enter description'onChange={handleDoctorDescription}/>
            {/*...................image pick..................... */}
            <div className='ContainerOfPickedImage'>
            <div style={{display:"flex", gap: "24px",height:"35px"}}>
            <SideBarDivText>Pick an Image:</SideBarDivText>
    ...
    fri
          ...
          <TimeField style={{width:"33%",marginLeft:"0.5%"}} className="form-control" value={startHoursIn} onChange={handlestarthour} />
          </div>
